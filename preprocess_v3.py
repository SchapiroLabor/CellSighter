...
def process_dataset(image_seg_pairs, root_path, quant_path, transposing, crop_input_size, crop_size, kfolds, lr, to_pad, blacklist, marker_path, sample_batch, aug, num_workers, size_data, batch_size, swap_train_val, val_size, max_epochs, split_test, hierarchy_match):
    ...
    for pair in image_seg_pairs:
        ...
        # Merge with the quantification data to get the encoded labels.
        # This will create NaNs for cells in the mask but not in the quantification data.
        full_mapping = all_cell_ids.merge(df2, on='cell_id', how='left')

        # Fill missing labels with -1 and ensure the column is integer type.
        full_mapping['cell_type'] = full_mapping['cell_type'].fillna(-1).astype(int)

        # Save the encoded labels to a text file in the 'cells2labels' directory.
        labels_output_path = os.path.join(root_path, 'CellTypes/cells2labels', f'{img_name}.txt')
        full_mapping['cell_type'].to_csv(labels_output_path, index=False, header=False)

        # The existing logic for creating a detailed mapping file for inspection can remain.
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Split the test set into this many integer parts for memory reasons."
             "This will add keys like 'test_set', 'test_set_part2', etc., to a single config file. "
